...
exports.parseArgv = function(argv = process.argv.slice(2)) {
  if (argv.length === 2) {
    if (argv[0] === '--preParsedJsonOptions') {
      return JSON.parse(argv[1]);
    } else if (argv[0] === '--preParsedBase64JsonOptions') {
      const json = Buffer.from(argv[1], 'base64').toString('utf8');
      debug('Parsed base64-encoded args into JSON: ', json);
